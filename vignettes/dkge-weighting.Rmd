...
DKGE has several weighting mechanisms, but they act on different axes and are
not interchangeable. This vignette identifies the unit each mechanism weights,
shows where it enters the estimator, and gives diagnostics for checking that a
small part of the data has not acquired unintended leverage.
...
DKGE operates with four complementary weighting layers:
...
The second layer is **subject-by-effect precision (`effect_weights`)**. It acts
on rows of the raw q-space subject moments. `"count"` uses trial counts stored
by `dkge_trial_subject()` or `dkge_subject(effect_n = ...)`; `"precision"`
uses explicit non-negative precision values. It is the layer for unequal cell
counts within subjects.

The third layer involves **subject weights (`w_method`, `w_tau`)**, which
rescale entire subject blocks based on their energy measured in the K-metric.
This layer offers Multiple Factor Analysis scaling (`"mfa_sigma1"`) and
Frobenius-based energy scaling (`"energy"`), alongside manual overrides.

The fourth layer comprises **transport weights (`sizes`)**, which shape the
optimal-transport alignment used to produce consensus maps. They operate after
the group basis is fitted and can respect parcel surface area or voxel density.
...
## Subject-by-Effect Precision

Suppose the four effect rows are cell means estimated from different numbers
of trials. Attach those counts to each subject, then request count weighting at
fit time:
...
effect_subjects <- lapply(seq_len(S), function(s) {
  dkge_subject(
    betas[[s]], design = designs[[s]], id = paste0("s", s),
    effect_n = effect_counts[[s]]
  )
})
data_effect <- dkge_data(effect_subjects)
...
range(fit_effect$pair_ess)
```

For effects \(c\) and \(c'\), subject \(s\) receives pair reliability
\(\sqrt{n_{sc}n_{sc'}}\). `fit_effect$pair_weight` records total reliability
mass and `fit_effect$pair_ess` records Kish effective sample size. A low
`pair_ess` warns that a pair is effectively determined by only a few subjects.
Explicit precision values use the same calculation through
`dkge_subject(effect_precision = ...)` and
`dkge_effect_weights("precision")`; zero precision excludes that
subject-effect row.

This weighting changes precision, not noise bias. If the cell means were
estimated from trialwise data, use `dkge_trial_subject()` and choose
`debias = "analytic"` or `"split_half"` separately. The complete 3 x 5 x 4
example is in `vignette("dkge-partial-effect-spaces")`.
