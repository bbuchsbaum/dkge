...
str(loso, max.level = 1)
```

- `loso$v`: medoid-aligned contrast values averaged across subjects.
- `loso$subject_values`: individual-subject contributions before aggregation.
- `loso$transport$plans`: transport matrices (subject clusters → medoid).

Plot the LOSO medoid map for quick inspection.

```{r loso-plot, fig.width=5, fig.height=4}
plot(loso$v, type = "h", lwd = 2, main = "LOSO contrast on medoid clusters",
