...
4. **Weighting**: Compute optional subject MFA weights (a frozen power iteration per subject) and voxel weights, accumulating the compressed covariance `Chat`. Under partial coverage, `none` and `mask` preserve **observed** pair mass; `rescale` returns an observed-pair mean, and `shrink` blends that mean toward its diagonal rather than restoring cohort mass.
