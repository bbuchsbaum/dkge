...
> `vignette("dkge-concepts")` first. It covers what DKGE actually
> decomposes (it pools a raw effect moment \(M\), then eigendecomposes
> \(K^{1/2}R^\top M R K^{1/2}\), so the kernel is a metric, not a basis), why
> identity-\(K\) is the unconstrained baseline that structured kernels should
> be compared against, and the four levels at which DKGE supports inference
> (component, contrast, feature, and between-subject term). The choices
> made below — `K = diag(q)`, `method = "loso"`, `dkge_component_stats()` —
...
When beta rows are estimated from unequal trial counts, construct each record
with `dkge_trial_subject()` instead. It retains cell estimates, counts,
effect-noise covariance, residual variances, and optional split halves, which
support `effect_weights` and `debias` without retaining the trial-by-feature
matrix. See `vignette("dkge-partial-effect-spaces")` for a runnable example.
...
| Partial cells and unequal trial counts | `dkge_trial_subject()`, `dkge_effect_weights()` |
| Between-subject group/trait tests | `dkge_between_rrr()`, `dkge_between_permute()` |
