...
```{r project}
subject_scores <- lapply(fit$Btil, function(Bts) t(Bts) %*% fit$K %*% fit$U)
str(subject_scores, max.level = 1)
```

Each subject score matrix has dimensions clusters × components. Average scores
for a quick component summary.

```{r score-plot, fig.height=4, fig.width=5}
