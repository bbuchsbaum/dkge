...
```{r performance, eval = requireNamespace("microbenchmark", quietly = TRUE)}
S <- 3; q <- 3; P <- 200
...
The example below calls `dkge:::sinkhorn_plan_cpp()`, an **internal** C++ function.
It is shown here for illustration only — the DKGE pipeline manages warm starts
automatically through its renderer objects. Do not rely on this function's
signature in production code; use `fit_mapper()` / `apply_mapper()` instead.
