...
test_that("zero precision is exactly structural missingness", {
  effects <- c("e1", "e2")
  B1 <- matrix(c(5, 6, 1, 2), 2, 2, byrow = TRUE,
               dimnames = list(effects, NULL))
  B2 <- matrix(c(2, 3, 4, 1), 2, 2, byrow = TRUE,
               dimnames = list(effects, NULL))

  zero_data <- dkge_data(list(
    make_reliability_subject(
      B1, "s1", precision = c(e1 = 0, e2 = 1)
    ),
    make_reliability_subject(
      B2, "s2", precision = c(e1 = 1, e2 = 1)
    )
  ))
  missing_data <- dkge_data(list(
    make_reliability_subject(
      B1, "s1", precision = c(e1 = 1, e2 = 1), observed_rows = 2
    ),
    make_reliability_subject(
      B2, "s2", precision = c(e1 = 1, e2 = 1)
    )
  ...
  zero_fit <- fit_one(zero_data)
  missing_fit <- fit_one(missing_data)

  expect_equal(zero_fit$Chat, missing_fit$Chat, tolerance = 1e-12,
               ignore_attr = TRUE)
  expect_equal(zero_fit$pair_counts, missing_fit$pair_counts,
               tolerance = 1e-12, ignore_attr = TRUE)
  expect_equal(zero_fit$pair_weight, missing_fit$pair_weight,
               tolerance = 1e-12, ignore_attr = TRUE)
  expect_equal(zero_fit$pair_ess, missing_fit$pair_ess,
               tolerance = 1e-12, ignore_attr = TRUE)
})
