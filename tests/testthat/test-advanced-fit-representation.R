...
test_that("audit mismatch probes no longer advertise a false block factor", {
  ...
  # The original audit fixtures reported errors near 0.237 and 0.534. These
  # deterministic probes exercise the same two invalid-factor mechanisms.
...
test_that("LOSO q-space contrast matches an independently pooled count oracle", {
  fixture <- make_advanced_qspace_fit()
  fit <- fixture$fit
  heldout <- 1L
  train <- setdiff(seq_along(fit$Braw), heldout)
  contrast <- setNames(c(0.5, -1, 0.75), fit$effects)

  numerator <- matrix(0, nrow(fit$K), ncol(fit$K))
  pair_weight <- matrix(0, nrow(fit$K), ncol(fit$K))
  for (s in train) {
    precision <- as.numeric(fit$effect_precision[[s]])
    reliability <- tcrossprod(sqrt(precision))
    numerator <- numerator + reliability * tcrossprod(fit$Braw[[s]])
    pair_weight <- pair_weight + reliability
  }
  pooled <- length(train) * numerator / pair_weight
  G_train <- Reduce(`+`, lapply(fit$subjects[train], function(subject) {
    crossprod(subject$design)
  }))
  diag(G_train) <- diag(G_train) + 1e-10
  R_train <- chol(G_train)
  Chat_minus <- fit$Khalf %*% t(R_train) %*% pooled %*%
    R_train %*% fit$Khalf
  Chat_minus <- (Chat_minus + t(Chat_minus)) / 2
  eig <- eigen(Chat_minus, symmetric = TRUE)
  Uminus <- fit$Kihalf %*%
    eig$vectors[, seq_len(fit$rank), drop = FALSE]
  ctil <- backsolve(R_train, contrast, transpose = FALSE)
  alpha <- t(Uminus) %*% fit$K %*% ctil
  A <- t(fit$Braw[[heldout]]) %*% R_train %*% fit$K %*% Uminus
  manual <- as.numeric(A %*% alpha)

  observed <- dkge_loso_contrast(fit, heldout, contrast)
  expect_equal(observed$R, R_train, tolerance = 1e-12,
               ignore_attr = TRUE)
  expect_equal(observed$basis, Uminus, tolerance = 1e-10,
               ignore_attr = TRUE)
  expect_equal(observed$alpha, alpha, tolerance = 1e-10,
               ignore_attr = TRUE)
  expect_equal(observed$v, manual, tolerance = 1e-10,
               ignore_attr = TRUE)
})
