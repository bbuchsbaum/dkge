...
- type:
  ...
  subjects to folds (default) - \`"custom"\`: User provides fold
...
- partition:
  ...
  nonoverlapping. Defining repeated or partial folds does not imply that
  every consumer can aggregate or label their outputs:
  subject-collapsing contrast and classification routines require
  exactly one assessment per subject.
...
assignments. Supply a list where each element specifies held-out subject
indices. The default exact-partition contract prevents accidental
overlap or omission.
