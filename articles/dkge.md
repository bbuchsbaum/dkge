...
fit$evals
#> [1] 527.8827 490.5156 392.4671
length(fit$subject_ids)
#> [1] 5
```
...
| [Concepts](https://bbuchsbaum.github.io/dkge/articles/dkge-concepts.md) | What DKGE actually decomposes, what K does, and the three levels of inference — read before your first fit |
...
| [Partial Effect Spaces](https://bbuchsbaum.github.io/dkge/articles/dkge-partial-effect-spaces.md) | Unbalanced cell coverage, effect precision, and trialwise debiasing |
...
**`dkge(betas, designs, K, rank)`** — main entry point. The first
argument may be a list of `dkge_subject` objects or a `dkge_data` bundle
(then `designs` is omitted). `K` is the `q × q` design kernel; pass
`design_kernel(factors, basis = "effect")` for structured factor
designs.
