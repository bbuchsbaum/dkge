...
dkge_between_rrr <- function(target,
  ...
  tol <- .dkge_validate_rank_tol(tol)
  if (!is.list(weights)) {
...
#' Validate a QR rank tolerance
#'
#' @param tol Candidate tolerance.
#' @return `tol` as a positive finite numeric scalar.
#' @keywords internal
#' @noRd
.dkge_validate_rank_tol <- function(tol) {
