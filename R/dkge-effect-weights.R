...
#'   heterogeneity is estimated from each effect row's spatial mean; the fit
#'   stores this assumption and all intermediate diagnostics.
#'
#'   The package-wide default remains `"none"` to preserve ordinary DKGE
#'   behavior. The included high-count-outlier simulation supports
#'   `"random_effects"` as the recommended opt-in policy for materially
#'   unbalanced trialwise designs; it is not activated silently for datasets
#'   that lack an explicit error-model choice.
#' @export
#' @examples
...
#' The resulting precision weights the whole subject effect row, so diagnostics
#' explicitly expose the scalar-summary assumption and dominance cap.
#'
#' @keywords internal
#' @noRd
.dkge_random_effects_precision <- function(dataset, spec, obs_masks = NULL,
  ...
  if (is.null(obs_masks)) {
    obs_masks <- replicate(dataset$n_subjects, rep(TRUE, q), simplify = FALSE)
  }
  masks <- lapply(indices, function(s) {
    mask <- as.logical(obs_masks[[s]])
    if (length(mask) != q) rep(TRUE, q) else mask
  })
  ...
  count_available <- vapply(indices, function(s) {
    n <- dataset$effect_n[[s]]
    !is.null(n) && length(n) == q && all(is.finite(n)) && all(n >= 0)
  }, logical(1))
