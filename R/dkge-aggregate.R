...
#'     \item{`saliences`}{`K %*% U`.}
#'     \item{`scores_feature`}{p x rank feature-space scores, `t(Yc) %*% saliences`.}
#'     \item{`singular_values`}{Per-component energies,
#'       `sqrt(colSums(scores_feature^2))`. For an unrotated fit these equal
#'       `sqrt(eig_values[seq_len(rank)])` exactly; after
#'       [dkge_aggregate_align()] they are the energies of the *rotated*
#'       components and are no longer sorted.}
#'     \item{`eig_values`}{Full length-q eigenvalue spectrum of `Chat`.}
#'     \item{`Chat`}{`K^{1/2} Yc t(Yc) K^{1/2}`. Both `Chat` and `eig_values`
#'       describe the data in the kernel metric and are invariant to the
#'       component rotation applied by [dkge_aggregate_align()]; only
#'       `U`, `saliences`, `scores_feature`, and `singular_values` are
#'       basis-dependent.}
#'   }
#' @export
